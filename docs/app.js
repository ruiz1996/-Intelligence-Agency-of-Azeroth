...
function homePage(){const reward=idlePreview(state,client.now()),rate=idleRates(state),task=currentTask();return {
  body:`<div class="home-layout"><section class="panel income-panel"><div><small>待领取金币</small><strong id="idle-gold">${money(reward.gold)}</strong><span id="idle-recruit">招募点 ${money(reward.recruit)}</span></div>${button('领取','claim','',false,'primary')}<small class="income-rate">每分钟 ${num(rate.gold)}金币 · ${num(rate.recruit)}招募点</small></section><section class="panel squad-panel"><div class="section-head"><h2>出战小队</h2>${button('布阵','nav','data-view="formation"',false,'quiet')}</div><div class="squad-grid">${state.formation.filter(Boolean).map(id=>button(`<img src="${art(id,true)}" alt="${HERO_BY_ID[id].name}"><span>${HERO_BY_ID[id].name}</span>`,'hero',`data-hero="${id}"`)).join('')}</div></section><section class="panel continue-card"><small>继续行动</small><h2>${esc(task.name)}</h2>${rewardTags(task)}</section><div class="home-shortcuts">${button('◇ 异常收容','category','data-kind="rogue"')}${button('↶ 时间回溯','nav','data-view="rebirth"')}</div></div>`,
...
function roguePage(){const pending=state.pendingBuff;if(pending&&!pending.options.includes(ui.selectedBuff))ui.selectedBuff=null;return {
  body:pending?`<h2 class="compact-title">${esc(TASKS[pending.taskId].name)}</h2><p class="muted">选择一份本轮强化</p><div class="choice-grid">${pending.options.map(id=>{const buff=BUFF_BY_ID[id];return button(`<strong>${buff.name}</strong><p>${esc(buffDisplay(id).summary)}</p>`,'selectBuff',`data-buff="${id}"`,false,`choice-card ${ui.selectedBuff===id?'selected':''}`);}).join('')}</div>${ui.selectedBuff?`<section class="panel selected-effect">${ownedBuffText(ui.selectedBuff)}</section>`:''}`:'<section class="panel empty"><h2>暂无待选强化</h2><p>完成新的收容任务，获得本轮强化。</p></section>',
...
function battlePage(){return {body:`<div class="battle-toolbar"><span id="battle-wave">第${battle.wave||1} / 3波</span><span id="battle-time">${battle.elapsed.toFixed(1)} / 90秒</span><div>${button(`${battle.speed}×`,'cycleSpeed','aria-label="切换倍速"')}${button('撤退','retreat','',false,'danger')}</div></div><div id="battle-announcement" class="battle-announcement">行动开始</div><div class="battlefield"><div class="enemy-grid" id="enemy-grid"></div><div class="battle-divider"><span>前排 ↑</span><span>后排 ↓</span></div><div class="ally-grid" id="ally-grid">${Array.from({length:6},(_,index)=>`<div class="empty-battle-slot" style="grid-column:${index%3+1};grid-row:${Math.floor(index/3)+1}"></div>`).join('')}</div></div>`,actions:button('战斗记录','battleLog')+button('本轮强化','buffArchive')};}
...
function modalContent(){
  ...
  switch(type){
    ...
    case 'probabilities': title='招募概率';body=`<p>每次消耗100招募点；十次消耗1000点。</p><p>16名特工各占6.25%，每次独立抽取，无保底。首次获得一星特工，重复获得10枚对应碎片。</p><div class="probability-list">${HEROES.map(h=>`${button(`<span>${h.name}</span><strong>6.25%</strong>`,'hero',`data-hero="${h.id}"`)}`).join('')}</div>`;break;
    ...
    case 'result': {const next=CATALOG[TASKS[d.taskId].kind][TASKS[d.taskId].index];title=d.outcome==='win'?'胜利':'失败';body=`<h3>${esc(TASKS[d.taskId].name)}</h3>${d.outcome==='win'?`<div class="reward-tags"><span>金币 +${esc(d.gold)}</span><span>招募点 +${esc(d.recruit)}</span></div>${d.item?itemDetails(d.item):''}${d.character?`<p>${HERO_BY_ID[d.character.hero].name} · ${d.character.new?'新特工':'+10碎片'}</p>`:''}`:'<p>调整阵容或强化装备后再试。</p>'}`;actions=button('返回任务','close')+(d.buff?button('选择强化','resultBuff','',false,'primary'):next&&d.outcome==='win'?button('下一任务','resultNext',`data-task="${next.id}"`,false,'primary'):button('再次挑战','start',`data-task="${d.taskId}"`,false,'primary'));if(d.outcome==='win'&&!d.buff&&next)body+=button('再次挑战','start',`data-task="${d.taskId}"`,false,'wide');break;}
...
function showWave(event){
  const heroes=Object.values(battle.units).filter(u=>u.side==='hero');
  battle.units=Object.fromEntries([...heroes,...event.units].map(u=>[u.id,u]));
  battle.wave=event.index;
  document.querySelector('#enemy-grid').replaceChildren();
  battle.currentFrame=[...(battle.currentFrame||[]).filter(u=>battle.units[u.id]?.side==='hero'),...event.units.map(u=>({id:u.id,hp:u.maxHp,shield:0}))];
  drawBattle();document.querySelector('#battle-wave').textContent=`第${event.index} / ${event.total}波`;
}
function applyFrame(units){for(const u of units){const card=document.getElementById('unit-'+u.id),meta=battle.units[u.id];if(!card||!meta)continue;card.classList.toggle('dead',u.hp<=0);card.querySelector('.hp-bar i').style.width=Math.max(0,u.hp/meta.maxHp*100)+'%';card.querySelector('.shield-bar i').style.width=Math.min(100,Math.max(0,u.shield/meta.maxHp*100))+'%';card.querySelector('.hp-label').textContent=`${meta.kind==='environment'?'余势 ':''}${num(u.hp)}/${num(meta.maxHp)}`;card.querySelector('.status-label').textContent=u.immune?'无敌':u.interfered?'技能干扰':u.debt>0?'醉伤 '+num(u.debt):u.shield>0?'盾 '+num(u.shield):'';}}
...
function playback(){
  if(!battle?.simulation)return;const now=performance.now();if(busy||sheet||now<battle.holdUntil){battle.last=now;return;}
  ...
  while(battle.index<sim.events.length&&sim.events[battle.index].time<=battle.elapsed){const e=sim.events[battle.index++];let text;
    if(e.type==='wave'){
      showWave(e);text=`第${e.index}波`;
      document.querySelector('#battle-announcement').textContent=text;battle.log.push({time:e.time,text});
      if(e.index>1){battle.elapsed=e.time;battle.holdUntil=now+(matchMedia('(prefers-reduced-motion: reduce)').matches?0:400);if(clock)clock.textContent=`${e.time.toFixed(1)} / 90秒`;break;}
    }
...
async function handle(action,el){
  ...
  if(['more','help','about','taskDetails','taskCatalog','heroPicker','chooseUpgrade','probabilities','exchangeHelp','buffArchive','pendingGear','battleLog','agentArchive'].includes(action)){openSheet(action);return;}
