...
const defaults={view:'home',kind:'idle',taskId:'R001',selectedHero:'kukalon',gearTab:'wear',gearFilterSlot:null,selectedSlot:0,quality:'all',query:'',page:0,recruitTab:'draw',breakthroughPhase:'attribute_2',breakthroughCount:1,selectedBuff:null};
...
async function connect(mode){
  ...
  try{const envelope=await client.load();state=envelope.state;
    if(state.schema===SCHEMA){if(!own(ui.selectedHero))ui.selectedHero=ownedHeroes()[0]?.id||'kukalon';if(!sessionStorage.getItem('ax-ui-v2'))selectProgress(ui.kind,false);}
...
function revealHeroes(result){return [...new Set([...(result.recruits||[]),...(result.character?[result.character]:[])].filter(x=>x.new&&HERO_BY_ID[x.hero]).map(x=>x.hero))];}
...
function heroesPage(){return {body:`<div class="section-head"><span>已拥有 ${ownedHeroes().length}/${HEROES.length}</span>${button('情报档案','agentArchive','',false,'quiet')}</div><div class="agent-roster">${ownedHeroes().map(h=>button(`<img src="${art(h.id,true)}" alt="${h.name}" loading="lazy"><strong>${h.name}</strong><small>${stars(h.id)}</small>`,'hero',`data-hero="${h.id}"`,false,'agent-tile')).join('')}</div>`,actions:ownedHeroes().length?button('出战阵容','nav','data-view="formation"',false,'primary'):button('前往招募','nav','data-view="recruit"',false,'primary')};}
...
function gearPage(){
  if(!ownedHeroes().length)return {body:'<section class="panel"><h2>原装备已保留</h2><p>基础特工退役补偿已发放。当前背包 '+state.items.length+' 件装备，招募新特工后即可重新穿戴。</p></section>',actions:button('前往招募','nav','data-view="recruit"',false,'primary')};
...
function breakthroughPage(){if(!ownedHeroes().length)return {body:'<p>招募特工后即可进行突破。</p>',actions:button('前往招募','nav','data-view="recruit"',false,'primary')};return breakthroughView(state,ui.selectedHero,ui.breakthroughPhase,ui.breakthroughCount,button);}
...
function modalContent(){
  ...
  switch(type){
    ...
    case 'recruits': title='特工报到';body=`<div class="recruit-results">${d.recruits.map(x=>HERO_BY_ID[x.hero]?`<article><img src="${art(x.hero,true)}" alt="${HERO_BY_ID[x.hero].name}"><strong>${HERO_BY_ID[x.hero].name}</strong><span>${x.new?'新特工 · 一星':'+10碎片'}</span></article>`:'<article><strong>已退役特工</strong><span>旧回执；补偿以当前存档为准</span></article>').join('')}</div>`;break;
    case 'idleGearResult': {
      title='获得挂机装备';
...
function battleStateText(meta,unit){
  ...
  if(meta.id==='mozhate')entries.push('灵魂残片：'+(unit.souls||0)+' / 5');
  if(meta.id==='juwoyaer')entries.push('神圣风暴：剩余'+(unit.verdictCharges||0)+'次 · 累计伤害 '+num(unit.verdictPool||0));
  if(unit.bladeReady)entries.push('熟练加班：下次刃舞提高15%');
  if(unit.chillUntil>battle.elapsed)entries.push('冰冷：剩余'+num(unit.chillUntil-battle.elapsed)+'秒');
...
function applyFrame(units){
  for(const u of units){
    ...
    if(u.hp>0){if(u.souls)statuses.push([String(u.souls),'灵魂残片']);if(u.verdictCharges)statuses.push([String(u.verdictCharges),'神圣风暴剩余次数']);if(u.chillUntil>battle.elapsed)statuses.push(['❄','冰冷']);if(u.bladeReady)statuses.push(['↑','熟练加班']);if(u.keyUntil>battle.elapsed)statuses.push(['⚿','非常棒的钥匙']);if(u.kkt)statuses.push(['↑','KKT攻击光环']);if(u.warIntent)statuses.push([String(u.warIntent),'战意']);if(u.cleaveCharges)statuses.push([String(u.cleaveCharges),'野兽顺劈剩余次数']);}
