import {
  ...
  heroStats,upgradeCost,salvageValue,canEquip,templateSlots,idleRates,idlePreview,
  rebirthPreview,allocationPreview,pointCost,act,
} from './core.js';
import {GameClient,cloudReady,selectedMode,setLocal,signOut,login} from './api.js';
import {esc,num,percent,art,series,itemName,itemStatsText,slotIcons,skillText,passiveText,mechanicText} from './presentation.js';

const root=document.querySelector('#app'),modal=document.querySelector('#modal'),toastNode=document.querySelector('#toast');
const titles={home:'情报局总部',tasks:'任务中心',heroes:'特工名录',formation:'出战阵容',gear:'装备整备',recruit:'特工招募',rogue:'收容强化',rebirth:'时间回溯',battle:'行动中'};
...
async function connect(mode){
  ...
  try{const envelope=await client.load();state=envelope.state;focus=state.focus||'main_hand';
    ...
    stamp();render();if(envelope.result)success(pendingAction||{},envelope.result);
...
function success(action,result){
  ...
  if(action.type==='abandon'){const target=sheet?.type==='retreat'?sheet.data:{next:'tasks'};stopBattle();transition(target.next||'tasks',target.patch||{},true);toast('已撤回行动');return;}
  if(result.outcome){ui.taskId=result.taskId;ui.kind=TASKS[result.taskId].kind;transition('tasks',{},true);openSheet('result',result);return;}
  if(result.recruits){render();openSheet('recruits',result,!!sheet);return;}
...
function homePage(){const reward=idlePreview(state,client.now()),rate=idleRates(state),task=currentTask();return {
  body:`<div class="home-layout"><section class="panel income-panel"><div><small>待领取金币</small><strong id="idle-gold">${money(reward.gold)}</strong><span id="idle-recruit">招募点 ${money(reward.recruit)}</span></div>${button('领取补给','claim','',false,'primary')}<small class="income-rate">每分钟 ${num(rate.gold)}金币 · ${num(rate.recruit)}招募点</small></section><section class="panel squad-panel"><div class="section-head"><h2>出战小队</h2>${button('布阵','nav','data-view="formation"',false,'quiet')}</div><div class="squad-grid">${state.formation.filter(Boolean).map(id=>button(`<img src="${art(id,true)}" alt="${HERO_BY_ID[id].name}"><span>${HERO_BY_ID[id].name}</span>`,'hero',`data-hero="${id}"`)).join('')}</div></section><section class="panel continue-card"><small>继续行动</small><h2>${esc(task.name)}</h2>${rewardTags(task)}</section><div class="home-shortcuts">${button('◇ 异常收容','category','data-kind="rogue"')}${button('↶ 时间回溯','nav','data-view="rebirth"')}</div></div>`,
...
function heroesPage(){return {body:`<div class="agent-roster">${HEROES.map(h=>button(`<img src="${art(h.id,true)}" alt="${h.name}" loading="lazy"><strong>${h.name}</strong><small>${own(h.id)?stars(h.id):'未招募'}</small>`,'hero',`data-hero="${h.id}"`,false,own(h.id)?'agent-tile':'agent-tile unowned')).join('')}</div>`,actions:`<span class="bar-note">已拥有 ${ownedHeroes().length}/${HEROES.length}</span>${button('出战阵容','nav','data-view="formation"',false,'primary')}`};}
function formationPage(){if(!formationDraft)formationDraft=[...state.formation];return {body:`<div class="formation-head"><span>点击位置调整特工</span><span>${formationDraft.filter(Boolean).length}/5人</span></div><div class="formation-grid">${formationDraft.map((id,index)=>button(`<span class="position-name">${index<3?'前排':'后排'} ${index%3+1}</span>${id?`<img src="${art(id,true)}" alt="${HERO_BY_ID[id].name}"><strong>${HERO_BY_ID[id].name}</strong>`:'<span class="empty-position">＋<small>空位</small></span>'}`,'formationSlot',`data-slot="${index}"`,false,'formation-slot')).join('')}</div><p class="muted">至少一名、最多五名特工。选择已上阵特工可交换位置。</p>`,actions:button('返回名录','nav','data-view="heroes"')+button('保存阵容','formation','',!draftDirty(),'primary')};}
...
function recruitPage(){let body=`<div class="tabs">${button('招募','recruitTab','data-tab="draw"',false,ui.recruitTab==='draw'?'selected':'')}${button('碎片兑换','recruitTab','data-tab="exchange"',false,ui.recruitTab==='exchange'?'selected':'')}</div>`,actions;
  if(ui.recruitTab==='draw'){
    body+=`<section class="recruit-banner panel"><div class="section-head"><h2>公会特别招募</h2>${button('概率详情','probabilities','',false,'quiet')}</div><div class="banner-portraits">${['lancelot','suxiaoyao','wudi'].map(id=>`<img src="${art(id)}" alt="${HERO_BY_ID[id].name}">`).join('')}</div><p>首次获得一星特工；重复获得10枚对应碎片。</p></section>`;
...
function roguePage(){const pending=state.pendingBuff;if(pending&&!pending.options.includes(ui.selectedBuff))ui.selectedBuff=null;return {
  body:pending?`<h2 class="compact-title">${esc(TASKS[pending.taskId].name)}</h2><p class="muted">选择一份本轮强化</p><div class="choice-grid">${pending.options.map(id=>{const buff=BUFF_BY_ID[id];return button(`<strong>${buff.name}</strong><p>${esc(buff.text)}</p>`,'selectBuff',`data-buff="${id}"`,false,`choice-card ${ui.selectedBuff===id?'selected':''}`);}).join('')}</div>`:'<section class="panel empty"><h2>暂无待选强化</h2><p>完成新的收容任务，获得本轮强化。</p></section>',
...
const resetDetails=()=>'<ul class="consequences"><li>保留特工、星级、碎片、招募点及装备收藏。</li><li>全部装备回到1级，本轮强化投入清零，之后分解不再返还这些投入。</li><li>任务进度与本轮收容强化清空。</li><li>当前金币重置为1600；下一轮加点和搜集倾向立即生效。</li></ul>';
...
function battlePage(){return {body:`<div class="battle-toolbar"><span id="battle-time">0.0 / 90秒</span><div>${button(`${battle.speed}×`,'cycleSpeed','aria-label="切换倍速"')}${button('撤退','retreat','',false,'danger')}</div></div><div id="battle-announcement" class="battle-announcement">行动开始</div><div class="battlefield"><div class="enemy-grid" id="enemy-grid"></div><div class="battle-divider"><span>前排 ↑</span><span>后排 ↓</span></div><div class="ally-grid" id="ally-grid">${Array.from({length:6},(_,index)=>`<div class="empty-battle-slot" style="grid-column:${index%3+1};grid-row:${Math.floor(index/3)+1}">空位</div>`).join('')}</div></div>`,actions:button('战斗记录','battleLog')+button('本轮强化','buffArchive')};}
...
function ownedBuffText(id){let text=BUFF_BY_ID[id].text;if(state.enhancedBuff===id)for(const rule of DATA.rogueEnhanceWhitelist.filter(w=>w.choice_id===id))text=text.replaceAll(percent(rule.from),percent(rule.to));return esc(text);}
function itemDetails(i){return `<div class="item-detail"><strong class="q${i.quality}">${esc(itemName(i))}</strong><p>${series(TEMPLATE_BY_ID[i.templateId].tier)} · ${QUALITY[i.quality]} · ${i.level}级</p><p>${itemStatsText(i)}</p>${i.affixes.map(a=>`<p>${AFFIX_NAMES[a.stat]||AFFIX_NAMES[a.key]||esc(a.stat||a.key)} +${percent(a.value)}</p>`).join('')}</div>`;}
...
function taskDetails(task){const r=task.rewards;return `${rewardTags(task)}<h3>敌方情报</h3><p>时限90秒 · 总生命 ${num(task.totalBaseHp)}</p><div class="enemy-preview">${task.enemies.map(e=>`<span>${e.type==='boss'?'首领 · 2×2':e.type==='elite'?'精英 · 2×1':'普通 · 1×1'}<small>攻击 ${num(e.atk)}</small></span>`).join('')}</div>${task.kind==='rogue'?`<h3>收容机制</h3><p>${mechanicText(task)}</p><p>本轮首次胜利后选择强化；重复挑战不再获得强化。</p>`:''}${task.kind==='gear'?`<h3>装备掉落</h3><p>${series(r.tier)}系列${r.nextTierChanceBp?`，有${r.nextTierChanceBp/100}%概率提升为${series(r.tier+1)}系列`:''}</p><p>${r.qualityChanceBp.map((p,i)=>QUALITY[i]+' '+p/100+'%').join(' · ')}</p>`:''}${!state.firsts.includes(task.id)?`<h3>首次完成奖励</h3><p>${task.kind==='idle'?`金币 ${num(r.historicalUnitBonusGold||0)} · 招募点 ${num(r.historicalUnitBonusRecruitPoints||0)}`:'招募点 100'}${r.historicalGuaranteedCharacter?` · ${esc(r.historicalGuaranteedCharacter)}`:''}${r.historicalSelectableEquipment?' · 行旅绿色装备自选':''}</p>`:''}<p class="muted">本轮回溯贡献：${state.runClears.includes(task.id)?'已计入':num(task.rebirthContributionSubunits/240)+'点'}。失败不发奖励，不消耗门票。</p>`;}
function modalContent(){
  const {type,data:d}=sheet;let title='',body='',actions=button('完成','close');
  switch(type){
    case 'more': title='更多';body=`<p class="account-status">${client.mode==='cloud'?'云档案已连接':'本机档案'} · 第${state.cycle}轮行动</p><div class="menu-list">${button('刷新档案','reload','',!!battle)}${button('导出存档','export')}${button('玩法帮助','help')}${button('版本信息','about')}${button('退出账号','logout','',!!battle)}</div>`;break;
    case 'help': title='玩法帮助';body='<p>领取后勤补给，完成资源任务提升挂机收入。装备任务提供装备，收容任务提供本轮强化。</p><p>在特工页安排最多五人的小队。在装备页选择部位、比较装备或强化，再前往任务出战。</p><p>完成第五个收容任务后，可规划下一轮永久加点并回溯。关闭网页会保留存档，未结算的战斗需要重新挑战。</p><p>保存失败时请点击“重试同步”。使用相同操作重试，不会重复扣费。</p>';break;
    case 'about': title='版本信息';body=`<p>手机布局 v0.2</p><dl class="stats-list"><div><dt>规则版本</dt><dd>${RULE_VERSION}</dd></div><div><dt>档案版本</dt><dd>${SCHEMA}</dd></div><div><dt>数值版本</dt><dd>${DATA.balanceRevision}</dd></div></dl><p class="muted">当前角色原画为候选素材，待正式美术替换。</p>`;break;
    ...
    case 'agent': {const h=HERO_BY_ID[d.hero],record=state.heroes[h.id],cost=[20,40,80,120][record.star-1];title=h.name;body=`<img class="portrait-large" src="${art(h.id)}" alt="${h.name}"><p class="stars">${record.owned?stars(h.id):'未招募'} · ${record.shards}枚碎片</p>${statsList(heroStats(state,h.id))}<h3>${h.active.name}<small>主动 · ${h.active.cooldownSeconds}秒</small></h3><p>${skillText(h)}</p><h3>${h.passive.name}<small>被动</small></h3><p>${passiveText(h)}</p><p class="muted">主手：${h.weapons.main.join(' / ')}<br>副手：${h.weapons.off.join(' / ')||'不可装备'}。技能不要求持有指定武器。</p>`;actions=button(record.star===5?'已满五星':`升星 · ${cost}碎片`,'star',`data-hero="${h.id}"`,!record.owned||record.star===5||record.shards<cost)+button('装备整备','heroGear',`data-hero="${h.id}"`,!record.owned,'primary');break;}
    ...
    case 'formationSlot': title=`${d.slot<3?'前排':'后排'} ${d.slot%3+1} · 选择特工`;body=`<div class="picker-list">${ownedHeroes().map(h=>{const position=formationDraft.indexOf(h.id);return button(`<img src="${art(h.id,true)}" alt=""><span>${h.name}<small>${position<0?'未上阵':`${position<3?'前排':'后排'} ${position%3+1} · ${position===d.slot?'当前位置':'点击交换'}`}</small></span>${position===d.slot?'✓':'›'}`,'placeHero',`data-hero="${h.id}" data-slot="${d.slot}"`);}).join('')}</div>`;actions=button('取消','close')+button('清空此位置','placeHero',`data-hero="" data-slot="${d.slot}"`,!formationDraft[d.slot],'danger');break;
    case 'discardFormation': title='阵容尚未保存';body='<p>离开后会放弃这次调整，已保存的阵容保持原样。</p>';actions=button('继续编辑','close')+button('放弃并离开','discardFormation','',false,'danger');break;
    case 'item': {const i=item(d.item);if(!i)return {title:'装备已不存在',body:'<p>这件装备已被分解或移除。</p>',actions};const owner=equipped(i.id),slots=templateSlots(TEMPLATE_BY_ID[i.templateId]).filter(slot=>canEquip(state,ui.selectedHero,i,slot));title='装备详情';body=itemDetails(i)+`<p>${owner?'穿戴者：'+HERO_BY_ID[owner].name:'存放在背包'}${i.locked?' · 已锁定':''}</p><div class="actions">${button(i.locked?'解除锁定':'锁定装备','lock',`data-item="${i.id}"`)}${button('分解','salvage',`data-item="${i.id}"`,!!owner||i.locked,'danger')}</div>`;actions=button('强化','upgradeSheet',`data-item="${i.id}"`)+(d.fromSlot?button('更换装备','changeSlot',`data-slot="${d.slot}"`,false,'primary'):button('比较穿戴','compare',`data-item="${i.id}" data-slot="${slots.includes(ui.gearFilterSlot)?ui.gearFilterSlot:slots[0]}"`,!slots.length||state.equipment[ui.selectedHero][slots[0]]===i.id,'primary'));if(d.fromSlot)body+=button('卸下装备','unequip',`data-slot="${d.slot}"`,false,'wide');break;}
    case 'compare': {const i=item(d.item);if(!i)return {title:'装备已不存在',body:'<p>请重新选择装备。</p>',actions};const slot=d.slot,old=item(state.equipment[ui.selectedHero][slot]),before=heroStats(state,ui.selectedHero);let after;try{after=heroStats(act(state,{type:'equip',hero:ui.selectedHero,item:i.id,slot}).state,ui.selectedHero);}catch(e){return {title:'无法穿戴',body:`<p>${esc(e.message)}</p>`,actions};}const main=item(state.equipment[ui.selectedHero][0]),owner=equipped(i.id);title='比较并替换';body=`<p>${HERO_BY_ID[ui.selectedHero].name} · ${SLOT_NAMES[slot]}</p><div class="comparison"><section><h3>当前</h3>${old?itemDetails(old):'<p>空位</p>'}</section><section><h3>替换为</h3>${itemDetails(i)}</section></div><h3>穿戴后的角色变化</h3><dl class="stats-list">${Object.keys(before).filter(k=>AFFIX_NAMES[k]&&typeof before[k]==='number'&&Math.abs(after[k]-before[k])>.000001).map(k=>`<div><dt>${AFFIX_NAMES[k]}</dt><dd class="${after[k]>before[k]?'positive':'negative'}">${statValue(k,before[k])} → ${statValue(k,after[k])}</dd></div>`).join('')||'<p>角色属性不变</p>'}</dl>${slot===0&&TEMPLATE_BY_ID[i.templateId].twoHand?'<p class="warning-text">双手武器会占用副手；当前副手装备将放回背包。</p>':''}${slot===1&&main&&TEMPLATE_BY_ID[main.templateId].twoHand?'<p class="warning-text">穿戴副手将把当前双手主手放回背包。</p>':''}${owner&&owner!==ui.selectedHero?`<p class="warning-text">将从${HERO_BY_ID[owner].name}身上取下这件装备。</p>`:''}`;actions=button('取消','close')+button('确认替换','equip',`data-item="${i.id}" data-slot="${slot}"`,false,'primary');break;}
    case 'upgrade': {const i=item(d.item);if(!i)return {title:'装备已不存在',body:'<p>请重新选择装备。</p>',actions};const count=d.count||1,p=upgradePlan(i,count);title='强化装备';body=itemDetails(i)+`<div class="stage-strip upgrade-counts">${[1,5,10,100].map(n=>button(`+${n}`,'upgradeCount',`data-count="${n}"`,false,count===n?'selected':'')).join('')}</div><p>当前金币 ${money(state.wallet.gold)}</p><p>计划提升${count}级，完整费用 ${formatInteger(p.total)}金币。</p><p class="${p.levels<count?'warning-text':''}">本次可提升${p.levels}级，${i.level} → ${BigInt(i.level)+BigInt(p.levels)}级。</p><p class="muted">金币不足时仅提升可支付的等级，按实际完成等级扣费。</p>`;actions=button('返回','close')+button(`强化 · ${formatInteger(p.paid)}金币`,'upgrade',`data-item="${i.id}" data-count="${count}"`,p.levels===0,'primary');break;}
    case 'chooseUpgrade': title='选择要强化的装备';body=`<div class="menu-list">${state.equipment[ui.selectedHero].filter(Boolean).map(id=>{const i=item(id);return button(`<span class="q${i.quality}">${esc(itemName(i))}</span><small>${i.level}级</small>`,'upgradeSheet',`data-item="${id}"`);}).join('')||'<p>尚未穿戴装备。可在背包选择装备后强化。</p>'}</div>`;break;
    case 'salvage': {const candidates=d.items.map(item).filter(Boolean),total=candidates.reduce((n,i)=>n+salvageValue(i,state),0n);title='确认分解';body=`<p>分解${candidates.length}件装备，返还 <strong>${formatInteger(total)}金币</strong>。</p><p>包含基础分解价和本轮实际强化投入。分解后装备无法恢复。</p><ul>${candidates.map(i=>`<li class="q${i.quality}">${esc(itemName(i))} · ${i.level}级</li>`).join('')}</ul>`;actions=button('取消','close')+button('确认分解','confirmSalvage','',!candidates.length,'danger');break;}
    case 'probabilities': title='招募概率';body=`<p>每次消耗100招募点；十次消耗1000点。</p><p>16名特工各占6.25%，每次独立抽取，无保底。首次获得一星特工，重复获得10枚对应碎片。</p><div class="probability-list">${HEROES.map(h=>`<p><span>${h.name}</span><strong>6.25%</strong></p>`).join('')}</div>`;break;
    case 'exchangeHelp': title='碎片兑换说明';body='<p>已拥有且未满星的特工：300招募点兑换10枚对应碎片。</p><p>满五星特工：多余碎片每枚兑换5招募点。点击特工查看本次费用或返还数量。</p>';break;
    case 'exchangeDetails': {const h=HERO_BY_ID[d.hero],record=state.heroes[h.id],max=record.star===5;title=h.name+' · 碎片兑换';body=`<img class="exchange-portrait" src="${art(h.id,true)}" alt="${h.name}"><p>当前${record.star}星 · ${record.shards}枚碎片</p><p>${max?`回收全部${record.shards}枚碎片，获得${record.shards*5}招募点。`:'消耗300招募点，获得10枚对应碎片。'}</p>`;actions=button('取消','close')+button(max?'确认回收':'确认兑换',max?'recycleShards':'exchange',`data-hero="${h.id}"`,max?!record.shards:BigInt(state.wallet.recruit)<300n*SCALE,'primary');break;}
    ...
    case 'result': {const next=CATALOG[TASKS[d.taskId].kind][TASKS[d.taskId].index];title=d.outcome==='win'?'行动完成':'行动未完成';body=`<h3>${esc(TASKS[d.taskId].name)}</h3>${d.outcome==='win'?`<div class="reward-tags"><span>金币 +${esc(d.gold)}</span><span>招募点 +${esc(d.recruit)}</span></div>${d.item?itemDetails(d.item):''}${d.character?`<p>${HERO_BY_ID[d.character.hero].name} · ${d.character.new?'新特工':'+10碎片'}</p>`:''}<p>${d.buff?'选择一份强化后继续行动。':d.first?'首次完成奖励已领取。':'奖励已领取。'}</p>`:'<p>本次未获得奖励，不消耗门票。调整阵容或强化装备后再试。</p>'}`;actions=button('返回任务','close')+(d.buff?button('选择强化','resultBuff','',false,'primary'):next&&d.outcome==='win'?button('下一任务','resultNext',`data-task="${next.id}"`,false,'primary'):button('再次挑战','start',`data-task="${d.taskId}"`,false,'primary'));if(d.outcome==='win'&&!d.buff&&next)body+=button('再次挑战','start',`data-task="${d.taskId}"`,false,'wide');break;}
    case 'buffArchive': title='本轮强化';body=`<div class="buff-archive">${state.buffs.map(id=>`<article><h3>${BUFF_BY_ID[id].name}${state.enhancedBuff===id?' · 已精修':''}</h3><p>${ownedBuffText(id)}</p></article>`).join('')||'<p>还没有获得本轮强化。</p>'}</div>`;break;
    case 'enhance': title='选择精修目标';body=`<div class="menu-list">${DATA.rogueEnhanceWhitelist.filter(w=>state.buffs.includes(w.choice_id)).map(w=>button(`<strong>${BUFF_BY_ID[w.choice_id].name}</strong><small>${esc(BUFF_BY_ID[w.choice_id].text)}</small>`,'enhanceTarget',`data-target="${w.choice_id}"`,false,d.target===w.choice_id?'selected':'')).join('')}</div>`;actions=button('返回','close')+button('确认精修','confirmEnhance','',!d.target,'primary');break;
    ...
    case 'pendingGear': title='领取自选装备';body=state.pendingEquipment.map(p=>`<h3>${esc(TASKS[p.taskId].name)} · ${p.group}</h3><div class="menu-list">${TEMPLATES.filter(t=>t.tier===p.tier&&t.group===({武器:'weapon',防具:'armor',首饰:'jewelry'}[p.group])).map(t=>button(`${esc(t.name)} · 绿色`,'selectEquipment',`data-task="${p.taskId}" data-template="${t.id}"`)).join('')}</div>`).join('')||'<p>奖励已领取完毕。</p>';break;
    case 'battleUnit': {const meta=battle?.units[d.unit],unit=battle?.currentFrame?.find(u=>u.id===d.unit);title=meta?.name||'单位详情';body=meta?`<p>生命 ${num(unit?.hp??meta.maxHp)} / ${num(meta.maxHp)} · 护盾 ${num(unit?.shield||0)}</p>${statsList(meta)}<p>${unit?.immune?'无敌中 · ':''}${unit?.interfered?'技能受到干扰 · ':''}${unit?.debt?'剩余醉伤 '+num(unit.debt):''}</p>${HERO_BY_ID[d.unit]?`<h3>${HERO_BY_ID[d.unit].active.name}</h3><p>${skillText(HERO_BY_ID[d.unit])}</p>`:''}`:'<p>战斗已结束。</p>';break;}
...
function renderSheet(){
  ...
  if(!modal.open)modal.showModal();modal.querySelector('.sheet-body').scrollTop=position;
}

function battleCard(u){return `<button data-action="battleUnit" data-unit="${u.id}" id="unit-${u.id}" class="battle-card ${u.side==='hero'?'ally':u.kind}" style="grid-column:${u.side==='hero'?u.slot%3+1:u.x+1} / span ${u.w};grid-row:${u.side==='hero'?Math.floor(u.slot/3)+1:u.y+1} / span ${u.h}" aria-label="查看${esc(u.name)}状态">${u.side==='hero'?`<img src="${art(u.id,true)}" alt="${u.name}">`:`<span class="enemy-symbol" aria-hidden="true">${u.kind==='boss'?'◈':u.kind==='elite'?'◆':'◇'}</span>`}<span class="battle-card-info"><strong>${esc(u.name)}</strong><span class="hp-bar"><i></i></span><span class="shield-bar"><i></i></span><small class="hp-label"></small><small class="status-label"></small></span></button>`;}
...
function drawBattle(){if(!battle?.simulation||ui.view!=='battle')return;for(const u of Object.values(battle.units))appendUnit(u);applyFrame(battle.currentFrame||battle.simulation.events.find(e=>e.type==='frame')?.units||[]);}
function applyFrame(units){for(const u of units){const card=document.getElementById('unit-'+u.id),meta=battle.units[u.id];if(!card||!meta)continue;card.classList.toggle('dead',u.hp<=0);card.querySelector('.hp-bar i').style.width=Math.max(0,u.hp/meta.maxHp*100)+'%';card.querySelector('.shield-bar i').style.width=Math.min(100,Math.max(0,u.shield/meta.maxHp*100))+'%';card.querySelector('.hp-label').textContent=`${num(u.hp)}/${num(meta.maxHp)}`;card.querySelector('.status-label').textContent=u.immune?'无敌':u.interfered?'技能干扰':u.debt>0?'醉伤 '+num(u.debt):u.shield>0?'盾 '+num(u.shield):'';}}
...
function beginBattle(challenge){
  stopBattle();toastNode.classList.remove('visible');toastNode.textContent='';battle={challenge,speed:2,elapsed:0,index:0,last:performance.now(),units:{},simulation:null,currentFrame:null,log:[]};
  transition('battle',{taskId:challenge.taskId,kind:TASKS[challenge.taskId].kind},true);
...
function playback(){
  if(!battle?.simulation)return;const now=performance.now();if(busy||sheet){battle.last=now;return;}
  battle.elapsed+=(now-battle.last)/1000*battle.speed;battle.last=now;const sim=battle.simulation;
  ...
  while(battle.index<sim.events.length&&sim.events[battle.index].time<=battle.elapsed){const e=sim.events[battle.index++];let text;
    if(e.type==='summon')appendUnit(e.unit);if(e.type==='frame'){battle.currentFrame=e.units;applyFrame(e.units);}
    ...
    if(text){document.querySelector('#battle-announcement').textContent=text;battle.log.push({time:e.time,text});}
  }
  if(battle.elapsed>=sim.duration){const challengeId=battle.challenge.id,outcome=sim.outcome;stopBattle();transition('tasks',{},true);mutate({type:'settle',challengeId,outcome});}
}
function bindInputs(){
...
async function handle(action,el){
  const d=el.dataset;if(busy)return;
  ...
  if(['more','help','about','taskDetails','taskCatalog','heroPicker','chooseUpgrade','probabilities','exchangeHelp','buffArchive','pendingGear','battleLog'].includes(action)){openSheet(action);return;}
  if(action==='hero'){openSheet('agent',{hero:d.hero});return;}
  ...
  if(action==='salvage'||action==='bulkSalvage'){openSheet('salvage',{items:action==='salvage'?[d.item]:[...marked]});return;}
  ...
  if(action==='start'){payload.taskId=d.task;}
  ...
  if(action==='confirmSalvage')Object.assign(payload,{type:'salvage',items:sheet.data.items});
  if(action==='confirmRetreat')Object.assign(payload,{type:'abandon'});
...
setInterval(()=>{if(!state||state.schema!==SCHEMA||ui.view!=='home')return;const r=idlePreview(state,client.now());const gold=document.querySelector('#idle-gold'),recruit=document.querySelector('#idle-recruit');if(gold)gold.textContent=money(r.gold);if(recruit)recruit.textContent='招募点 '+money(r.recruit);},1000);
