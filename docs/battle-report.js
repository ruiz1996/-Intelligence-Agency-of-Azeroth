...
export function battleReport(sim,through=Infinity){
  ...
  for(const rows of [total,...waves])for(const r of Object.values(rows))r.shieldRemaining=Math.max(0,r.shieldCreated-r.shield-r.shieldSpent-r.shieldExpired);
  // Per-wave remaining includes shields carried in, rather than a negative creation balance.
  const balances=Object.fromEntries(Object.keys(total).map(id=>[id,0]));
  for(const rows of waves)for(const [id,r] of Object.entries(rows)){r.shieldCarried=balances[id];r.shieldRemaining=Math.max(0,balances[id]+r.shieldCreated-r.shield-r.shieldSpent-r.shieldExpired);balances[id]=r.shieldRemaining;}
  return {version:1,heroes,total,waves,duration:Math.min(sim.duration,through),partial:through<sim.duration};
}
